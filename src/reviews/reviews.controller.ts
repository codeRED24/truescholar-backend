...
export class ReviewsController {
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        college_images: { type: 'array', items: { type: 'string', format: 'binary' } },
  ...
  async create(
    ...
    const collegeImages = files?.filter((f) => f.fieldname === 'college_images');

    // Upload each file to S3 (if present) and set URL fields on DTO
    if (profilePic) {
      // @ts-ignore - multer File shape from fastify-multer
      const url = await this.fileUploadService.uploadFile(profilePic as any, 'reviews/profile_pictures');
      createReviewDto.profile_picture_url = url;
    }

    if (degreeCert) {
      const url = await this.fileUploadService.uploadFile(degreeCert as any, 'reviews/documents');
      createReviewDto.degree_certificate_url = url;
    }

    if (markSheet) {
      const url = await this.fileUploadService.uploadFile(markSheet as any, 'reviews/documents');
      createReviewDto.mark_sheet_url = url;
    }

     if (studentId) {
      const url = await this.fileUploadService.uploadFile(markSheet as any, 'reviews/student-ids');
    ...
    if (collegeImages && collegeImages.length) {
      ...
      for (const img of collegeImages) {
        const url = await this.fileUploadService.uploadFile(img as any, 'reviews/college_images');
        urls.push(url);
      }
  ...
  update(
    ...
    @Body(new ValidationPipe({ whitelist: true })) updateReviewDto: UpdateReviewDto,
  ) {
